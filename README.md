...
📌 Overview
The ABC Tyres eBilling Pipeline is an end‑to‑end automated document delivery system designed to ingest invoice PDFs from SFTP, enrich them with metadata from Synapse, apply business delivery rules stored in Azure SQL, classify and rename the documents, and deliver final ZIP bundles back to SFTP for downstream customer distribution.

This solution replaces traditional on‑premise delivery systems with a modern, cloud‑native architecture built on Azure PaaS, ensuring scalability, observability, and operational resilience.
...
Metadata used to determine BillTo/SoldTo accounts and delivery rules
...
Synapse workspace secured with managed identity
...
📈 Monitoring & Logging
Python logs written to logs/pipeline.log
